// Todas as constantes que são utilizadas no programa para calcular a trajetória do robô até a bola
...
function iniciarPrograma(){
    ...
    for(let i = 0; i < 365; i++){
        ...
        vetorForcaXbola.push(0.046 * aceleracaoXBola);
        vetorForcaYbola.push(0.046 * aceleracaoYBola);
        vetorForcaXrobo.push(2.8*aceleracaoRobo*cos[i]);
        vetorForcaYrobo.push(2.8*aceleracaoRobo*sin[i]);
        ...
        if(roboX[i] >= parseFloat(vetorBolaX[i]) - 0.1115 && roboX[i] <= parseFloat(vetorBolaX[i]) + 0.1115 && roboY[i] >= parseFloat(vetorBolaY[i]) - 0.1115 && roboY[i] <= parseFloat(vetorBolaY[i]) + 0.1115){
            ...
            // vetorForcaXinterbola = vetorForcaXbola[i] + vetorForcaXrobo[i]
            // vetorForcaYinterbola = vetorForcaYbola[i] + vetorForcaYrobo[i]
            // vetorForcaXinterrobo = 
...
function grafico(){
    ...
    var data = [movObX2];
    var layout = {
        title: 'Altura da bola pelo eixo X'
    };
    Plotly.newPlot('movObliX', data, layout);
    ...
    var data = [movObY2];
    var layout = {
        title: 'Altura da bola pelo eixo Y'
    };
    Plotly.newPlot('movObliY', data, layout);
